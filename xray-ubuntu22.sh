...
clear
echo -e "[ ${green}INFO${NC} ] XRAY Core Installation for Ubuntu $UBUNTU_VERSION"
...
# Install dependencies optimized for Ubuntu 22.04
echo -e "[ ${green}INFO${NC} ] Installing dependencies for Ubuntu $UBUNTU_VERSION..."
apt update -y

# Ubuntu 22.04 specific packages
if [ "$UBUNTU_VERSION" = "22.04" ] || [ "$UBUNTU_VERSION" = "24.04" ]; then
    apt install -y python3 python3-pip python3-venv
    apt remove -y python2.7 python3.9 2>/dev/null || true
else
...
apt install -y curl wget git unzip net-tools \
    socat xz-utils gnupg2 dnsutils lsb-release \
...
wget -q -O /usr/local/bin/xray "https://github.com/XTLS/Xray-core/releases/download/v1.8.11/Xray-linux-64.zip"
unzip -q -o /usr/local/bin/xray -d /usr/local/bin/ 2>/dev/null || true

# If download from GitHub fails, use fallback
if [ ! -f "/usr/local/bin/xray" ]; then
    echo -e "[ ${yellow}INFO${NC} ] Using fallback Xray binary..."
...
if [ "$USE_WILDCARD" = true ] && [ -f "/etc/letsencrypt/live/$main_domain/fullchain.pem" ]; then
    echo -e "[ ${green}INFO${NC} ] Using Let's Encrypt wildcard certificate..."
    ...
    echo -e "[ ${green}INFO${NC} ] Generating SSL certificate with ACME.sh..."
...
echo -e "[ ${green}SUCCESS${NC} ] XRAY Core installed successfully on Ubuntu $UBUNTU_VERSION"
echo -e "[ ${green}INFO${NC} ] Domain: $domain"
echo -e "[ ${green}INFO${NC} ] Wildcard: $USE_WILDCARD"

# Cleanup
rm -f /root/xray-ubuntu22.sh
