...
if [ "$USE_WILDCARD" = true ]; then
    ...
    if [ -f "/usr/local/etc/xray/xray.crt" ] && [ -f "/usr/local/etc/xray/xray.key" ]; then
        echo -e "[ ${green}INFO${NC} ] Wildcard certificate found and ready to use"
    else
        echo -e "[ ${red}ERROR${NC} ] Wildcard certificate not found!"
        echo -e "[ ${green}INFO${NC} ] Generating fallback certificate..."
    ...
    echo -e "[ ${green}INFO${NC} ] Generating SSL Certificate..."
    mkdir /root/.acme.sh
...
sleep 1
rm -r xray2-wildcard.sh

echo -e "[ ${green}SUCCESS${NC} ] XRAY Core with Wildcard Support Installed Successfully!"
echo -e "[ ${green}INFO${NC} ] Domain: $domain"
echo -e "[ ${green}INFO${NC} ] Wildcard Status: $USE_WILDCARD"
